...
/* 	GÖREV 3:
    - Sayfanın en üstünde global değişken olarak tanımlanmış bir sayilar dizisi bulunmaktadır. Bu dizi içinde 0 ile 1000 arasında rasgele oluşturulmuş tam sayılar ve ondalıklı sayılar bulunmaktadır. Bu diziyi kullanarak aşağıdakileri uygulayın:
      ...
      3f. `tekraredensayilar` adında bir dizi oluşturun. sayilar dizisi içerisindeki bazı sayılar birden fazla kere yazılmış. sayilar dizisi 
      içerisinde birden fazla kez yazılmış sayıları tespit ederek kaç kere tekrar edildiğini belirten bir string 
...
sayilar.forEach(element => {
  if (element % 3 == 0) ucetambolunenler.push(element);
...
ucebolunenlerintoplami = ucetambolunenler.reduce(function(acc, num) {
...
besyuzdenkucuksayilar = sayilar.filter((item) => { return item < 500; });
...
/* 	let kontrol=[];
  tekraredensayilar=[];
    for (let i=0;i<sayilar.length;i++){
      let sayi=sayilar[i];
      let tekrarSayisi=sayilar.filter(function(x){return x==sayi}).length;
      if(!kontrol.includes(sayi) && tekrarSayisi>1){
